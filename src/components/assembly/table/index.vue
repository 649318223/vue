...
-->
<template>
  <el-table :data="configTable.tableData" border @selection-change="tableChange">
    ...
    <template v-for="item in configTable.tableHead">
      <el-table-column v-if="item.type=='slot'" :key="item.headID" :width="item.width" :prop="item.headID" :label="item.label" align="center">
        <template slot-scope="scope">
          <slot :name="item.slotName" :data="scope.row"></slot>
        </template>
      </el-table-column>
      <el-table-column v-else :key="item.headID" :width="item.width" :prop="item.headID" :label="item.label" align="center"></el-table-column>
    </template>
  </el-table>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    //复选框选中时触发
    tableChange(val){
      this.$refs.multipleTable.bb();
          var tableDataSelect = {
              id : val.map(item => item.id),
              data : val
          }
          this.$emit("update:tableSelectData", tableDataSelect);
    },
    //获取表格配置
    getTableConfig() {
